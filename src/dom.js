...
const dom = (() => {
    ...
    const renderShipSelection = () => {
        ...
        rotateButton.onclick = () => {
            ...
            rotateButton.textContent = `Rotate Ship (${isHorizontal ? 'Horizontal' : 'Vertical'})`;
        };
    ...
    const handlePlayerAttack = (x, y) => {
        ...
        if (game.checkGameEnd()) {
            const winner = game.players[0].gameboard.allShipsSunk() ? "Computer" : "Player";
            showGameOver(winner);
            return;
        }
    ...
    const setupEventListeners = () => {
        ...
        playerBoardElement.addEventListener('mouseover', (event) => {
            ...
            for (let i = 0; i < selectedShip.length; i++) {
                ...
                const previewCell = playerBoardElement.querySelector(`[data-x="${previewX}"][data-y="${previewY}"]`);
                if (previewCell) {
                    previewCell.classList.add('ship-preview');
                    if (canPlace) {
                        previewCell.style.setProperty('background', 'rgba(0, 255, 0, 0.7)', 'important');
                    } else {
                        previewCell.style.setProperty('background', 'rgba(255, 0, 0, 0.7)', 'important');
                    }
                }
            }
        });
        ...
        playerBoardElement.addEventListener('click', (event) => {
            ...
            try {
                ...
                if (game.players[0].gameboard.ships.length === 5) {
                    shipSelectionElement.innerHTML = '<h3>All ships placed! Click on enemy board to attack!</h3>';
                }
            } catch (error) {
                alert(error.message);
            }
